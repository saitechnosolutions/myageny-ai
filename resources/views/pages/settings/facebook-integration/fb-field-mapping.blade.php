...
<div class="fb-fields-card">
    <div class="fb-fields-head">
        <div>
            <h3 class="fb-fields-title">Field Mapping</h3>
            <p class="fb-fields-copy">Map Facebook campaign fields to your CRM fields so lead data is saved in the correct place. Each Facebook field should be paired with one CRM field before continuing.</p>
        </div>
        <div class="fb-fields-chip">{{ isset($camid) ? count($camid) : 0 }} Campaigns to Map</div>
    </div>
            ...
            @if (isset($camid))
                <form action="">
                    <div class="fb-fields-grid">
                        @foreach ($camid as $cam)
                            @php
                                $fields = App\Models\LeadFormField::all();
                                $fbfields = App\Models\CampaignFields::where('campaign_id', $cam)->get();
                                $campaignMaster = App\Models\CampaignMaster::where('id', $cam)->first();
                            @endphp

                            <div class="fb-fields-panel">
                                <h4 class="fb-fields-panel-title">Map Fields For {{ $campaignMaster->campaign_name }}</h4>
                                <p class="fb-fields-panel-sub">Choose one Facebook field and one CRM field per row to define how this campaign’s leads should be imported.</p>
                                            ...
                                            @foreach ($fbfields as $field)
                                                <select class="crm-field-select fb-fields-select">
                                                    ...
                                                    @foreach ($fields as $crmField)
                                                        <option value="{{ $crmField->id }}">{{ $crmField->label }}</option>
                                                    @endforeach
                                                </select>
                                            @endforeach
                                        </div>
                    ...
                    <div class="fb-fields-actions">
                        <div class="fb-fields-note">Complete all field pairs before continuing to user assignment.</div>
                        <div>
                            ...
                            <button type="button" class="crm-btn crm-btn-primary" id="map_multiple_fields" disabled>Map Fields</button>
                        </div>
                    </div>
                </form>
            @else
                <div class="fb-fields-empty">
                    ...
                    <p>The selected campaign has already been integrated and no additional field mapping is required right now.</p>
                </div>
            @endif
        </div>
    </div>
