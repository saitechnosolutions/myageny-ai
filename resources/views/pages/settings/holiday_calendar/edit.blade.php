...
<div class="holiday-page">
    <div class="eob-topbar">
        <div>
            <div class="eob-title">Edit Holiday</div>
            <div class="eob-breadcrumb">HRMS > Holiday Calendar > Edit</div>
        </div>
    ...
    <div class="eob-body">
        <div class="holiday-card" style="max-width:760px; margin:0 auto; width:100%;">
            <div class="holiday-card-head">
                <div>
                    <h2 style="margin:0;font-size:18px;font-weight:800;color:#121212;">Edit Holiday</h2>
                    <p style="margin:6px 0 0;color:#7c7c7c;font-size:13px;">Update the saved holiday date or reason.</p>
                </div>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('settings.holiday-calendars.update', $holiday) }}">
            @csrf
            @method('PUT')
            <div class="crm-form-wrap">
                ...
                <div class="crm-form-body">
                    <div>
                        ...
                        <input type="date" name="holiday_date" class="crm-input" value="{{ old('holiday_date', $holiday->holiday_date->format('Y-m-d')) }}" required>
                    </div>
                    <div>
                        <label class="crm-label">Reason for Holiday <span class="req">*</span></label>
                        <textarea name="reason" class="crm-textarea" required>{{ old('reason', $holiday->reason) }}</textarea>
                    </div>
                </div>
                <div class="crm-form-foot">
                    <a href="{{ route('settings.holiday-calendars.index') }}" class="crm-btn crm-btn-ghost">Cancel</a>
                    <button type="submit" class="crm-btn crm-btn-primary">Update Holiday</button>
                </div>
            </div>
        </form>
    </div>
