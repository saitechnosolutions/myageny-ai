...
<main class="main-content">
            ...
            <table class="crm-table">
                ...
                @forelse($categories as $cat)
                    <tr>
                        ...
                        <td><span class="crm-count-badge">{{ $cat->attributes_count }}</span></td>
                        <td>{{ $cat->created_at->format('d M Y') }}</td>
                        <td class="text-right">
                            <button class="crm-icon-btn" onclick="openEdit({{ $cat->id }}, '{{ addslashes($cat->name) }}')">✏️</button>
                            <form action="{{ route('settings.product-category.destroy', $cat) }}" method="POST" style="display:inline"
    ...
    <div id="editModal" class="crm-modal-overlay" style="display:none">
        <div class="crm-modal">
            <div class="crm-modal-header"><h3>Edit Product Category</h3><button onclick="closeModal('editModal')">✕</button></div>
